...
if VERSION < v"0.7.0-DEV.3172"
    # Signature of replace() was changed in https://github.com/JuliaLang/julia/pull/25165
    function Base.replace(str::AbstractString, pat_repl::Pair; count::Integer=typemax(Int))
        pattern, repl = pat_repl
        replace(str, pattern, repl, count)
    end
end
